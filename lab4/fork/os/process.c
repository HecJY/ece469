...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    /*
    //STUDENT: Initialize the PCB’s page table here
    set to 0 for all pagetable entries & all processes
    */
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  //---------------------------------------------------------
  //the page number is 4 at the beginning4 pages for user code and .
  pcb->npages = 6;
  for(i = 0; i < 4; i ++){
    page = MemoryAllocPage();
    //check to see whether the page is valid
    if(page == MEM_FAIL){
      printf("In process fork, allocation of page failed\n");
      exitsim();
    }
    //asign the allocated page's pte to the pagetable
    pcb->pagetable[i] = MemorySetupPte(page);
  }
  //system stack
  /*allocate a new page
set pcb system stack area w.r.t. this new page
stackframe = set to the bottom (highest addr) of the sys. stack area (4-byte*/
  //1 page for system stack

  page = MemoryAllocPage();
  if(page == MEM_FAIL){
    printf("Page allocation for system stack failed in Process Fork\n");
    exitsim();
  }

  //set to the bot of the sys
  pcb -> sysStackArea = page * MEM_PAGESIZE;

  //user stack
  /*allocate another new page for user stack
  set pcb pagetable entry for this page
  (remember user stack is located at the max page number)
  allocate 4 pages for user code & global data
  (4 lowest page numbers)*/
  page = MemoryAllocPage();
  if(page == MEM_FAIL){
    printf("Page allocation for User stack failed in Process Fork\n");
  }
  //to the max page
  pcb->pagetable[(MEM_L1TABLE_SIZE - 1)] = MemorySetupPte(page);
  ...
  /*stackframe[PTBASE] = ?
stackframe[PTBITS],stackframe[PTSIZE]*/
...
void ProcessRealFork(){
  ...
  // make sure freepcbs is not empty
  if (AQueueEmpty(&freepcbs)){
    printf("FATAL error: error in free processes!\n");
    exitsim();
  }
  ...
  if (AQueueRemove (&(child->l)) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not remove link!\n");
    exitsim();
  }

  //Loop through currentPCB’s L1 page table and mark valid ones as READONLY
  for (i = 0; i < MEM_L1TABLE_SIZE; i++){
    if (currentPCB->pagetable[i] & MEM_PTE_VALID){
      currentPCB->pagetable[i] |= MEM_PTE_READONLY;
      refcounter_inc(currentPCB->pagetable[i] & MEM_PTE_MASK);
    }
  }
  ...
  // allocate a new page for system stack and set pcb→sysStackArea
  if ((new_page = MemoryAllocPage()) == MEM_FAIL){
    printf("FATAL ERROR: could not malloc space for new page!\n");
    exitsim();
  }
  child->sysStackArea = new_page * MEM_PAGESIZE;
  ...
  //Fix the sysstack pointer and currentSavedFrame to the new stack area
  ...
  //Set childpcb’s status to runnable
  ProcessSetStatus(child, PROCESS_STATUS_RUNNABLE);

  //grab child pcb link
  if ((child->l = AQueueAllocLink(child)) == NULL){
    printf("FATAL ERROR: could not get link!\n");
    exitsim();
  }

  //insertlast (runqueue, child pcb link)
  if (AQueueInsertLast(&runQueue, child->l) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not insert link!\n");
    exitsim();
  }
  ...
  //****Q4
  //Printing out the entries of parent PCB
  parent_pid = GetPidFromAddress(currentPCB); 
  printf("Parent Process(%d): sysStackPtr: %x\n", parent_pid, currentPCB->sysStackPtr);
  printf("Parent Process(%d): sysStackArea: %x\n", parent_pid, currentPCB->sysStackArea);
  printf("Parent Process(%d): current saved frame: %x\n", parent_pid, currentPCB->currentSavedFrame);
  for (i = 0; i < MEM_L1TABLE_SIZE; i++){
    if (currentPCB->pagetable[i]) {
      printf("Parent Process(%d): %d's pte: %d\n", parent_pid, i, currentPCB -> pagetable[i]);
    }
  }

  printf("\n");

  //Printing out the entries of child PCB
  child_pid = GetPidFromAddress(child);
  printf("Child Process(%d): sysStackPtr: %x\n", child_pid, child->sysStackPtr);
  printf("Child Process(%d): sysStackArea: %x\n", child_pid, child->sysStackArea);
  printf("Child Process(%d): current saved frame: %x\n", child_pid, child->currentSavedFrame);
  for (i = 0; i < MEM_L1TABLE_SIZE; i++){
    if (child->pagetable[i]) {
      printf("Child Process(%d): %d's pte: %d\n", child_pid, i, child->pagetable[i]);
    }
  }
}
